...
class TD3_Trainer():
    ...
    def update(self, batch_size, deterministic, eval_noise_scale, reward_scale=10., gamma=0.9, soft_tau=1e-2):
        ...
        new_action,  _= self.policy_net.evaluate(state, last_action, hidden_in)  # no noise, deterministic policy gradients
        new_next_action, _ = self.target_policy_net.evaluate(next_state, action, hidden_out) # clipped normal noise
...
frame_idx   = 0
batch_size  = 8  # each sample contains an episode for lstm policy
